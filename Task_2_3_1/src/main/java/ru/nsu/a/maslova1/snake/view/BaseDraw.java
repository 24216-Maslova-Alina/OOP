...
public class BaseDraw {
    ...
    /**
     * Вычисляет оптимальный размер одной клетки игрового поля в зависимости от текущего размера холста.
    ...
    protected double getCellSize() {
        ...
        if (width <= 0 || height <= 0) return 25.0;

        return Math.min(width / GameConfig.COLS, height / GameConfig.ROWS);
    }

    /**
     * Вычисляет смещение по оси X, необходимое для отображения игрового поля ровно по центру холста.
     *
    ...
    /**
     * Вычисляет смещение по оси Y, необходимое для отображения игрового поля ровно по центру холста.
     *
